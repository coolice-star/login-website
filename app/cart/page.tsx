...
import { ArrowLeft, ShoppingCart, Heart, Coins, Star, TrendingUp, RefreshCw, ShoppingBag, Trash2, Plus, Minus } from "lucide-react"
...
export default function CartPage() {
  ...
  const loadCartItems = () => {
    try {
      ...
      if (savedCart) {
        ...
        const items = cartProductIds.map((id: number) => {
  ...
  const removeFromCart = (productId: number) => {
    try {
      ...
      const cartProductIds = updatedItems.map(item => item.product.id);
  ...
  const toggleFavorite = (productId: number) => {
    try {
      ...
      if (favorites.has(productId)) {
        favoritesList = favoritesList.filter((id: number) => id !== productId);
        setFavorites(prev => {
  ...
  const getTotalPoints = () => {
    return cartItems.reduce((total, item) => total + (item.product.pointsPrice * item.quantity), 0);
  };
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-purple-50 flex items-center justify-center">
        <Card className="w-full max-w-md">
          <CardContent className="p-8 text-center">
            ...
            <h2 className="text-lg font-semibold mb-2">加载购物车中...</h2>
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-gray-600">管理您要购买的商品</p>
              </div>
            </div>
          ...
          {cartItems.length === 0 ? (
            <Card>
              <CardContent className="p-8 text-center">
                ...
                <p className="text-gray-500 mb-6">快去添加您喜欢的商品吧！</p>
                        ...
                        <div className="flex-1">
                          <div className="flex items-start justify-between">
                            <div>
                              ...
                              <h3 
                                className="font-semibold text-gray-900 hover:text-blue-600 transition-colors cursor-pointer line-clamp-2"
                                onClick={() => goToProductDetail(item.product.id)}
                              >
                                {item.product.name}
                              </h3>
                              ...
                              <div className="flex items-center gap-4 mt-1 text-sm text-gray-600">
                                ...
                                <div className="flex items-center gap-1">
                                  ...
                                  <span>{item.product.sales}+已售</span>
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              >
                                <Heart className={`h-4 w-4 ${favorites.has(item.product.id) ? 'fill-red-500 text-red-500' : 'text-gray-400'}`} />
                              </Button>
                              <Button
                                variant="ghost"
                                size="icon"
          ...
          {cartItems.length > 0 && (
            <Card className="mt-6 bg-blue-50 border-blue-200">
              <CardContent className="p-4 text-center">
                <p className="text-sm text-blue-700 mb-3">购物车共有 {cartItems.length} 种商品</p>
